...
- Replayed the same 32 768-point residual benchmark with the new JSONL sink. Default traversal (`COVERTREEX_SCOPE_CHUNK_TARGET=0`) now clocks **1058.58 s build / 0.208 s query (4 919 q/s)** for PCCT vs. **2.79 s / 11.09 s (92.3 q/s)** for GPBoost (`benchmark_residual_default_20251107_run2.{log,jsonl}`). The traversal clamp held (`traversal_scope_chunk_max_members` never exceeded 16 384 and 63/64 dominated batches recorded trims) but adjacency still saw **`conflict_scope_chunk_max_members≈8.4 M`** because the dense builder processed each mega-scope as a single chunk. Steady-state `conflict_adj_scatter_ms` averaged 54 ms with spikes past 70 ms.
- Turning chunking on (`COVERTREEX_SCOPE_CHUNK_TARGET=8192`, artefact `benchmark_residual_scope8192_20251107.{log,jsonl}`) kept both traversal- and conflict-side members at 8 192, reduced traversal time by ~41 % (`traversal_ms` median 11.1 s vs. 21.7 s), and cut build time to **625.19 s**. Query latency rose slightly (**0.252 s / 4 063 q/s**), and the first dominated batch still spent **364 ms** scattering across ~1.9 k chunk shards, so we have proof that segmentation volume rather than raw membership is the next bottleneck. GPBoost landed at **2.94 s / 12.81 s (79.9 q/s)**.
- The JSONL sink (`--log-file`) is now part of the benchmark flow, so we can diff steady-state traversal/adjacency metrics without scraping stdout.

###### Next steps — 2025-11-07

1. **Carry the clamp into adjacency.** Traversal now hard-caps at 16 384 but adjacency still processes 8 M-member mega-scopes whenever chunking is disabled, yielding `conflict_scope_chunk_segments=1` and 54 ms scatter. Honour the new `scope_chunk_max_segments` override in the dense NumPy builder so we always split residual scopes before adjacency. Target: `conflict_scope_chunk_segments≈256` and `<50 ms` scatter in the default run.
2. **Retune the chunked path.** Chunked traversal improves build time but produces hundreds of shards and a 364 ms scatter spike on the first dominated batch. Teach `_chunk_ranges_from_indptr` to merge tail shards once a scope falls below a per-range threshold so steady-state segment counts stay in the “low hundreds” without losing the 8 192-member guardrail. Use the JSONL telemetry to validate (`scope_chunk_segments`, `conflict_adj_scatter_ms`).
3. **Refresh documentation + tables.** Once (1) and (2) hold, regenerate the residual + Euclidean tables in `docs/CORE_IMPLEMENTATIONS.md` and attach the new log artefacts so auditors see both the numeric recovery (≥4 k q/s) and the supporting JSONL traces.
4. **Tier-C coverage remains CPU-only.** Keep the async refresh harness under NumPy/Numba thread pools; GPU/JAX smoke tests stay deprecated until the backend resurfaces.

###### Implementation plan — 2025-11-06

- **Scope segmentation & caps** — Implement real segmentation once the sparse traversal kernel lands. Reuse `_chunk_ranges_from_indptr` inside `build_conflict_graph_numba_dense` so `chunk_target` produces filtered CSR shards rather than a no-op and gate the total shard count behind `COVERTREEX_SCOPE_CHUNK_MAX_SEGMENTS` to stop the builder from emitting thousands of tiny tiles. Feed per-segment radii into the filter and accumulate telemetry for chunk hits, capped memberships, and backpressure bytes; surface them via `ConflictGraphTimings` and the `batch_insert` logger. On the traversal side, add level- or parent-chain splits in `_collect_residual_scopes_streaming` so we prune memberships before the conflict graph sees 16 k-entry scopes.

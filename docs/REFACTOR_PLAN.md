...
4. **Trim 32 k traversal wall time.** *In progress — residual scan caps landed 2025‑11‑08.* The new grid/Hilbert runs still spend 250–490 ms per dominated batch in `traversal_ms` while `conflict_graph_ms` stays under 20 ms. We now reuse `COVERTREEX_SCOPE_CHUNK_TARGET` to bound both residual scope sizes and scan depth, so dominated batches stop after ~8 k candidates even when the ladder keeps radii tiny. Next steps: (a) opportunistically cache per-level candidate masks between back-to-back dominated batches, and (b) explore chunk-level SIMD prefilters (norm bounds) so `_collect_scopes_dense` can drop obviously-far nodes before distance accumulation.

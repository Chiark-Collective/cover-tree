...
- **Gold-standard harness.** `benchmarks/run_residual_gold_standard.sh` locks in the canonical 32 768-point residual benchmark (dense traversal, no chunking, diagnostics off) that yielded the 56.09 s / 0.229 s result. Running the script regenerates `bench_residual.log` with the exact defaults so we always have a fresh “gold” artefact for regressions.

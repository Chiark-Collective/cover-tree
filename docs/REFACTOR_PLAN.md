...
- **Lookup + scope-cap refresh (2025‑11‑08).** Captured a full 32 768-point stream with caps enabled and Gate‑1 audit on, producing `docs/data/residual_gate_profile_32768_caps.json` (2,096,128 samples) and the per-level medians in `docs/data/residual_scope_caps_32768.json`. The new CLI knobs (`--residual-scope-cap-output`, `--residual-scope-cap-percentile`, `--residual-scope-cap-margin`) write that JSON straight from `cli.queries`, so deriving fresh caps no longer requires hand-editing.

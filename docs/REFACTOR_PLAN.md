...
4. **Trim 32 k traversal wall time.** *Pending.* The new grid/Hilbert runs still spend 250–490 ms per dominated batch in `traversal_ms` while `conflict_graph_ms` stays under 20 ms. Next steps: (a) feed `scope_chunk_target` through the Euclidean streaming path so we can cap candidate scans at 8 192 without reordering the tree, (b) opportunistically cache per-level candidate masks between back-to-back dominated batches, and (c) explore chunk-level SIMD prefilters (norm bounds) so `_collect_scopes_dense` can drop obviously-far nodes before distance accumulation.
